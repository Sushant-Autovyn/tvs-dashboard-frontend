...
@Component({
  selector: 'app-solved-tickets',
  standalone: true,
  imports: [CommonModule, TicketTableComponent],
  templateUrl: './solved-tickets.html',
  styleUrl: './solved-tickets.css'
})
export class SolvedTicketsComponent implements OnInit, OnDestroy {
  tickets: Ticket[] = [];
  loading = true;
  ...
  fetch(): void {
    ...
    this.sub = this.ticketService.getTickets().subscribe({
      next: (data) => { this.tickets = data.filter(t => t.status === 'resolved'); this.loading = false; },
